...
export default function ReviewScreen() {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            try {
                ...
                // ✅ Fetch balance (only once per chain change)
                const bal = await provider.getBalance(walletAddress);
                setBalance(ethers.utils.formatEther(bal));

                // ✅ Fetch gas price (approx fee)
                const gas = await ChainManager.getGasPrice(chainId);
                setGasFee(gas);

                // ✅ USD estimate (live with amount)
                const ethToUsd = 3500; // replace with live API later
  ...
  try {
    ...
    // ⚡ Trigger transaction immediately
    const tx = await TransactionService.sendTransaction(receiver, amount, chainId);

    if (tx && tx.hash) {
      // ✅ Immediately navigate to Activity page with pending tx
      setTimeout(() => {
        router.push({
      ...
      Alert.alert("Transaction Failed ❌", tx?.error || "Unknown error");
    }
  } catch (err: unknown) {
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.header}>
                <TouchableOpacity
                    onPress={() => !sending && router.back()}
                    disabled={sending}
                    style={[styles.backButton, sending && { opacity: 0.4 }]}
                >
                    <Ionicons name="chevron-back" size={28} color={sending ? "#999" : "#000"} />
                </TouchableOpacity>
            ...
            <View style={styles.bottomButtons}>
                <TouchableOpacity
                    ...
                    onPress={() => !sending && router.push("/")}
                    disabled={sending}
                >
                    <Text style={[styles.cancelText, sending && { color: "#999" }]}>
                        Cancel
                    </Text>
                </TouchableOpacity>
